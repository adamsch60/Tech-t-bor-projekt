...
*/
	...
	app.get('/get_history', function(req, res) {
			...
			db.Sequelize.query("SELECT case when p1id="+req.user.id+" then User2.username else User1.username end AS enemy ,case when winner = 1 and p1id="+req.user.id+" then 'you' else case when winner = 2 and p1id="+req.user.id+" then 'enemy' else case when winner = 1 and p2id="+req.user.id+" then 'enemy' else case when winner = 2 and p2id="+req.user.id+" then 'you' else 'tie' end end end end AS `winner1`, strftime('%Y-%m-%d', date) AS `date1`, case when p1id="+req.user.id+" then 'you' else 'enemy' end AS `starter`, case when p1id="+req.user.id+" then round(Match.elo_diff1) else round(Match.elo_diff2) end AS elo_diff FROM `Matches` AS `Match`, 'Users' AS 'User1','Users' AS 'User2' WHERE User1.id=p1id AND User2.id=p2id AND (`Match`.`p1id` = "+req.user.id+" OR `Match`.`p2id` = "+req.user.id+") ORDER BY 2 DESC;", { type: sequelize.QueryTypes.SELECT }).then(results => {
